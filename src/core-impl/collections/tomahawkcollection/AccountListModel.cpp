...
#include "AccountListModelNode.h"

#include "/home/llg/Programacao/tomahawk/src/libtomahawk/AtticaManager.h"
#include "/home/llg/Programacao/tomahawk/src/libtomahawk/accounts/ResolverAccount.h"
...
AccountListModel::loadData()
{
    ...
    foreach ( AccountFactory* fac, factories )
    {
        ...
        m_accounts << new AccountListModelNode( fac );
    ...
    foreach ( const Attica::Content& content, fromAttica )
    {
        ...
        if ( AtticaManager::instance()->hasCustomAccountForAttica( content.id() ) )
        {
            ...
            if ( acct )
            {
                m_accounts << new AccountListModelNode( acct );
                const int removed = allAccounts.removeAll( acct );
            }
        } else
        {
            m_accounts << new AccountListModelNode( content );
   ...
   foreach ( Account* acct, allAccounts )
   {
       ...
       if ( qobject_cast< ResolverAccount* >( acct ) && !qobject_cast< AtticaResolverAccount* >( acct ) )
           m_accounts << new AccountListModelNode( qobject_cast< ResolverAccount* >( acct ) );
       else
           m_accounts << new AccountListModelNode( acct );
   }
...
AccountListModel::data( const QModelIndex& index, int role ) const
{
    ...
    const AccountListModelNode* node = m_accounts.at( index.row() );
    // This is a top-level item. 3 cases

    switch ( node->type )
    {
        case AccountListModelNode::FactoryType:
        {
        ...
        case AccountListModelNode::AtticaType:
        {
        ...
        case AccountListModelNode::ManualResolverType:
        case AccountListModelNode::UniqueFactoryType:
        {
            if ( role == RowTypeRole )
            {
                if ( node->type == AccountListModelNode::ManualResolverType )
                    return TopLevelAccount;
                else
            ...
            {
                return node->type == AccountListModelNode::ManualResolverType;
            }

            Account* acct = 0;
            if ( node->type == AccountListModelNode::ManualResolverType )
                acct = node->resolverAccount;
            else if ( node->type == AccountListModelNode::UniqueFactoryType )
            ...
            if ( !acct )
            {
                Q_ASSERT( node->type == AccountListModelNode::UniqueFactoryType );
        ...
        case AccountListModelNode::CustomAccountType:
        {
           ...
           const bool hasAttica = false; //= !node->atticaContent.id().isEmpty();
...
bool
AccountListModel::setData( const QModelIndex& index, const QVariant& value, int role )
{
    if ( !index.isValid() || !hasIndex( index.row(), index.column(), index.parent() ) )
        return false;

    AccountListModelNode* node = m_accounts.at( index.row() );
    ...
    if ( role == CheckboxClickedRole )
    {
        ...
        if ( node->type == AccountListModelNode::FactoryType )
        {
    ...
    if ( role == CustomButtonRole )
    {
        ...
        else if ( node->type == AccountListModelNode::ManualResolverType )
        {
    ...
    if ( role == RatingRole )
    {
        ...
        Q_ASSERT( node->type == AccountListModelNode::AtticaType );
...
void
AccountListModel::accountAdded( Account* account )
{
    ...
    {
        AccountListModelNode* n = m_accounts.at( i );
        ...
        if ( n->factory == factory && !(resolver) ) // Specifically ignore manual resolvers, as the user added them directly
        {
...
//    // Ok, just a plain resolver. add it at the end
    if ( ResolverAccount* resolver = qobject_cast< ResolverAccount* >( account ) )
    {
        ...
        m_accounts << new AccountListModelNode( resolver );
        endInsertRows();
...
void
AccountListModel::accountStateChanged( Account* account , Account::ConnectionState )
{
    // Find the factory this belongs up, and update
    for ( int i = 0; i < m_accounts.size(); i++ )
    {
        AccountListModelNode* n = m_accounts.at( i );
        if ( n->type != AccountListModelNode::FactoryType )
        {
            ...
            if ( ( n->type == AccountListModelNode::UniqueFactoryType && n->accounts.size() && n->accounts.first() == account ) ||
                 ( n->type == AccountListModelNode::AtticaType && n->atticaAccount && n->atticaAccount == account ) ||
                 ( n->type == AccountListModelNode::ManualResolverType && n->resolverAccount && n->resolverAccount == account ) )
            {
                const QModelIndex idx = index( i, 0, QModelIndex() );
...
void
AccountListModel::accountRemoved( Account* account )
{
    ...
    for ( int i = 0; i < m_accounts.size(); i++ )
    {
        ...
        if ( ( n->type == AccountListModelNode::FactoryType && n->factory == factory ) ||
             ( n->type == AccountListModelNode::UniqueFactoryType && n->accounts.size() && n->accounts.first() == account ) )
        {
        ...
        if ( n->type == AccountListModelNode::AtticaType && n->atticaAccount && n->atticaAccount == account )
        {
            n->atticaAccount = 0;
            found = true;
        }
        ...
        if ( n->type == AccountListModelNode::ManualResolverType && n->resolverAccount && n->resolverAccount == account )
        {
...
int
AccountListModel::rowCount( const QModelIndex& ) const
{
