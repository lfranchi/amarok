...
class UtilsObject : public QObject
{
    Q_OBJECT
public:
    UtilsObject( DelegateAccountConfigWrapper* w ) : QObject( w ), m_w( w ) {}
...
private:
    DelegateAccountConfigWrapper* m_w;
...
createAccountFromFactory( Tomahawk::Accounts::AccountFactory* factory, QWidget* parent )
{
    ...
    if( account->configurationWidget() )
    {
        ...
        dialog->show();
#else
        DelegateAccountConfigWrapper dialog( account->configurationWidget(), account->aboutWidget(), QString("%1 Config" ).arg( account->accountFriendlyName() ), parent );
        QWeakPointer< DelegateAccountConfigWrapper > watcher( &dialog );
...
openAccountConfig( Tomahawk::Accounts::Account* account, QWidget* parent, bool showDelete )
{
    ...
    {
#ifndef Q_OS_MAC
        DelegateAccountConfigWrapper dialog( account->configurationWidget(), account->aboutWidget(), QString("%1 Configuration" ).arg( account->accountFriendlyName() ), parent );
