...
AccountListFactoryWrapperDelegate::AccountListFactoryWrapperDelegate( QObject* parent )
...
void
AccountListFactoryWrapperDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    Account* acc = qobject_cast< Account* >( index.data( AccountListFactoryWrapper::AccountRole ).value< QObject* >() );
    Q_ASSERT( acc );
...
QSize
AccountListFactoryWrapperDelegate::sizeHint(const QStyleOptionViewItem&, const QModelIndex&) const
{
...
bool
AccountListFactoryWrapperDelegate::editorEvent( QEvent* event, QAbstractItemModel*, const QStyleOptionViewItem&, const QModelIndex& index )
{
    ...
    if ( event->type() == QEvent::MouseButtonPress )
    {
        ...
        if ( me->button() == Qt::LeftButton && m_cachedConfigRects.contains( index ) && m_cachedConfigRects[ index ].contains( me->pos() ) )
        {
            ...
            Account* acct = qobject_cast< Account* >( index.data( AccountListFactoryWrapper::AccountRole ).value< QObject* >() );
