...
#include "/home/llg/Programacao/tomahawk/src/libtomahawk/utils/TomahawkUtils.h"
#include "/home/llg/Programacao/tomahawk/src/libtomahawk/utils/Logger.h"
#include "/home/llg/Programacao/tomahawk/src/libtomahawk/utils/AnimatedSpinner.h"
#include "/home/llg/Programacao/tomahawk/src/libtomahawk/utils/Closure.h"
...
AccountListDelegate::AccountListDelegate( QObject* parent )
...
QSize
AccountListDelegate::sizeHint( const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
    AccountListModel::RowType rowType = static_cast< AccountListModel::RowType >( index.data( AccountListModel::RowTypeRole ).toInt() );
    ...
    if ( rowType == AccountListModel::TopLevelAccount || rowType == AccountListModel::UniqueFactory || rowType == AccountListModel::CustomAccount )
    {

        return QSize( 200, m_accountRowHeight );
    }
    else if ( rowType == AccountListModel::TopLevelFactory )
    {
        // Make more space for each account we have to show.
        AccountFactory* fac = qobject_cast< AccountFactory* >( index.data( AccountListModel::AccountData ).value< QObject* >() );
        if ( fac->isUnique() )
            return QSize( 200, m_accountRowHeight );

        const QList< Account* > accts = index.data( AccountListModel::ChildrenOfFactoryRole ).value< QList< Tomahawk::Accounts::Account* > >();
        const QSize s = QSize( 200, m_accountRowHeight + 12 * accts.size()-1 );

        if ( s != m_sizeHints[ index ] )
            const_cast< AccountListDelegate* >( this )->sizeHintChanged( index ); // FU KTHBBQ
...
void
AccountListDelegate::paint ( QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
    QStyleOptionViewItemV4 opt = option;
    initStyleOption( &opt, index );
    ...
    const AccountListModel::RowType rowType = static_cast< AccountListModel::RowType >( index.data( AccountListModel::RowTypeRole ).toInt() );
    ...
    const bool hasConfigWrench = index.data( AccountListModel::HasConfig ).toBool();
    ...
    bool canDelete = index.data( AccountListModel::CanDeleteRole ) .toBool();
    if ( rowType == Tomahawk::Accounts::AccountListModel::TopLevelFactory )
    {
        const QList< Account* > accts = index.data( AccountListModel::ChildrenOfFactoryRole ).value< QList< Tomahawk::Accounts::Account* > >();
    ...
    else if ( rowType == AccountListModel::UniqueFactory )
    {
    ...
    QRect textRect;
    const bool canRate = index.data( AccountListModel::CanRateRole ).toBool();
    if ( canRate )
    {
    ...
    // author
    QString author = index.data( AccountListModel::AuthorRole ).toString();
    ...
    // description
    QString desc = index.data( AccountListModel::DescriptionRole ).toString();
    ...
    if ( index.data( AccountListModel::CanRateRole ).toBool() )
    {
        // rating stars
        const int rating = index.data( AccountListModel::RatingRole ).toInt();
...
//             QRect r( runningEdge, opt.rect.top() + PADDING, m_ratingStarPositive.width(), m_ratingStarPositive.height() );
            ...
            const bool userHasRated = index.data( AccountListModel::UserHasRatedRole ).toBool();
        ...
        QString count = tr( "%1 downloads" ).arg( index.data( AccountListModel::DownloadCounterRole ).toInt() );
        painter->setFont( descFont );
...
int
AccountListDelegate::drawAccountList( QPainter* painter, QStyleOptionViewItemV4& opt, const QList< Account* > accts, int rightEdge ) const
{
...
bool
AccountListDelegate::editorEvent( QEvent* event, QAbstractItemModel* model, const QStyleOptionViewItem& option, const QModelIndex& index )
{
    ...
    if ( event->type() == QEvent::MouseButtonPress )
    {
        ...
        if ( me->button() == Qt::LeftButton && m_cachedConfigRects.contains( index ) && m_cachedConfigRects[ index ].contains( me->pos() ) )
        {
            ...
            const AccountListModel::RowType rowType = static_cast< AccountListModel::RowType >( index.data( AccountListModel::RowTypeRole ).toInt() );
            if ( rowType == AccountListModel::TopLevelAccount ||
                 rowType == AccountListModel::CustomAccount )
            {
                Account* acct = qobject_cast< Account* >( index.data( AccountListModel::AccountData ).value< QObject* >() );
    ...
    } else if ( event->type() == QEvent::MouseButtonRelease || event->type() == QEvent::MouseButtonDblClick )
    {
        ...
        if ( checkRectForIndex( option, index ).contains( me->pos() ) )
        {
            ...
            return model->setData( index, newState, AccountListModel::CheckboxClickedRole );
        }
        else if ( m_cachedButtonRects.contains( index ) && m_cachedButtonRects[ index ].contains( me->pos() ) )
        {
            ...
            model->setData( index, true, AccountListModel::CustomButtonRole );
        }
    }
    ...
    if ( m_cachedStarRects.contains( index ) )
    {
        ...
        if ( fullStars.contains( me->pos() ) )
        {
            ...
            {
                model->setData( index, whichStar, AccountListModel::RatingRole );
            }
...
void
AccountListDelegate::drawRoundedButton( QPainter* painter, const QRect& btnRect, bool red ) const
{
...
int
AccountListDelegate::drawStatus( QPainter* painter, const QPointF& rightTopEdge, Account* acct, bool drawText ) const
{
    QPixmap p;
    QString statusText;
    const Account::ConnectionState state = Account::Disconnected;//acct->connectionState();
    ...
    if ( state == Account::Connecting )
    {
        if ( !m_connectingSpinners.contains( acct ) )
        {
            ...
            _detail::Closure* closure = new _detail::Closure( anim, SIGNAL( requestUpdate() ), const_cast<AccountListDelegate*>(this), SLOT( doUpdateIndexWithAccount( Tomahawk::Accounts::Account* ) ), C_ARG( Tomahawk::Accounts::Account*, acct ) );
...
void
AccountListDelegate::drawCheckBox( QStyleOptionViewItemV4& opt, QPainter* p, const QWidget* w ) const
{
    QStyle* style = w ? w->style() : QApplication::style();
...
void
AccountListDelegate::drawConfigWrench ( QPainter* painter, QStyleOptionViewItemV4& opt, QStyleOptionToolButton& topt ) const
{
    const QWidget* w = opt.widget;
    QStyle* style = w ? w->style() : QApplication::style();
...
QRect
AccountListDelegate::checkRectForIndex( const QStyleOptionViewItem& option, const QModelIndex& idx ) const
{
    QStyleOptionViewItemV4 opt = option;
...
void
AccountListDelegate::startInstalling( const QPersistentModelIndex& idx )
{
...
void
AccountListDelegate::doneInstalling ( const QPersistentModelIndex& idx )
{
...
void
AccountListDelegate::doUpdateIndex( const QPersistentModelIndex& idx )
{
    emit update( idx );
}
...
void
AccountListDelegate::doUpdateIndexWithAccount( Account* account )
{
    ...
    for ( int i = 0; i < m_model->rowCount(); i++ )
    {
        ...
        else if ( rowType == AccountListModel::TopLevelFactory || rowType == AccountListModel::UniqueFactory )
        {
            const QList< Account* > accts = index.data( AccountListModel::ChildrenOfFactoryRole ).value< QList< Tomahawk::Accounts::Account* > >();
